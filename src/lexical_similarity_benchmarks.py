...
def analyze_performance(predicted_sims, actual_sims, word_pairs):
  assert len(predicted_sims) == len(actual_sims)
  assert len(actual_sims) == len(word_pairs)
  diffs = [math.abs(p[0] - p[1]) for p in zip(predicted_sims, actual_sims)]
  word_to_diffs = zip(words, diffs)
  word_to_diffs.sort()
  print(word_to_diffs)
  return word_to_diffs
...
def run_benchmark(benchmark_name, embedding_map, decontextualized_embeddings=None):
  ...
  predicted_sims, actual_sims, word_pairs, oovs = [], [], [], []
  total_items = 0
  ...
  perf = analyze_performance(predicted_sims, actual_sims, word_pairs)
  if num_skipped:
