...
def get_average(embedding_table, embedding_dim, vocab, word_to_count_map, log_sum):
  norms = []
  table = sstable.SSTable(
      embedding_table,  # Proto2
      wrapper=sstable.TableWrapper(embedding_pb2.TokenEmbedding.FromString))
  ...
  for key in keys:
    ...
    count = math.log(word_to_count_map[key])
    sum_embedding += embedding * (count / log_sum)
...
def main(_):
  vocab = None
  word_to_count_map_file = '/usr/local/google/home/agoldie/data/glove/word_to_count_map.pkl'
  with open(word_to_count_map_file, 'rb') as f:
    word_to_count_map = pickle.load(f)
  # First, get the log of the sum of word instances.
  log_sum_instances = -1
  if FLAGS.vocab_file:
  ...
  if FLAGS.mean_embedding_file:
    ...
    mean_embedding, norms = get_average(FLAGS.input_embedding_table, FLAGS.embedding_dim, vocab, word_to_count_map, log_sum_instances)
