...
flags.DEFINE_string(
    'output_dir', '',
    'Output directory in which to write out the sstables for each layer.'
)
...
def main(argv):
  ...
  with open(FLAGS.embedding_map, 'rb') as f:
    layer_to_word_to_decontextualized_embedding = pickle.load(f)

  for layer in layer_to_word_to_decontextualized_embedding:
    output_sstable = os.path.join(FLAGS.output_dir, 'decontextualized-embeddings-' + str(layer) + '.sstable')
    with sstable.SortingBuilder(output_sstable) as builder:
      for word in layer_to_word_to_decontextualized_embedding[layer]:
        print(layer, word) #, len(word_to_layer_to_decontextualized_embedding[word][layer)))
        decontextualized_embedding = layer_to_word_to_decontextualized_embedding[layer][word]
