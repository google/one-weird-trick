...
flags.DEFINE_string(
    'input_dir', '',
    'Directory of sstables in which pre-trained word embeddings are stored.'
)
...
def main(_):
  ...
  input_tables = os.listdir(input_dir)
  input_tables = [os.path.join(input_dir, t) for t in input_tables]
  input_tables = [t for t in input_tables if 'subtracted' in t and 'removed' not in t]
  ...
  for input_table in input_tables:
    ...
    if os.path.exists(output_file):
      print('Skipping...')
      continue
    vocab_name = input_table_base[len('subtracted-'):-len('.sstable')]
    vocab_base = vocab_name + '.pkl'
    vocab_file = os.path.join(vocab_dir, vocab_base)
    if not os.path.exists(vocab_file):
      vocab_name += 'wout-stopwords'
      vocab_base = vocab_name + '.pkl'
      vocab_file = os.path.join(vocab_dir, vocab_base)
