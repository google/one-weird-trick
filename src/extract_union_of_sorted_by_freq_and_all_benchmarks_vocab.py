...
flags.DEFINE_string(
    'sorted_by_freq_dir', '',
    'Directory containing vocabulary subsets'
)
...
def main(_):
  ...
  if not os.path.isdir(FLAGS.sorted_by_freq_dir):
    print('FLAGS.sorted_by_freq_dir must be a directory.')
    exit(0)
  sorted_by_freq = os.listdir(FLAGS.sorted_by_freq_dir)
  for vocab_file in sorted_by_freq:
    if not vocab_file.endswith('.pkl'):
      continue
    vocab_file = os.path.join(FLAGS.sorted_by_freq_dir, vocab_file)
    ...
    base = 'union-all-benchmarks-' + os.path.basename(vocab_file)
